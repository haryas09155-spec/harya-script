...
local REQUIRED_KEY = "Haryas_Script"
...
local old = guiParent:FindFirstChild("Haryas-ScriptKeyLoader")
if old then
    old:Destroy()
end

local screenGui = Instance.new("ScreenGui")
screenGui.Name = "Haryas-ScriptKeyLoader"
...
titleText.Text = "Haryas-Script - Key System"
...
infoLabel.Text = "A key is required to access Haryas script. Open the key link, get your key, then enter it below."
...
local KEY_URL = "https://discord.gg/AzZhAZDZHJ"
...
local function authorizeMainHub()
    ...
    if okGuid and type(guid) == "string" and guid ~= "" then
        ...
        token = "Haryas-Script-" .. tostring(os.time()) .. "-" .. tostring(math.random(100000, 999999))
    end
...
local function loadMainHub()
    ...
    if not ok then
        ...
        warn("[Haryas-Script KeyLoader] HttpGet main.lua failed:", res)
        return
    end
    ...
    task.defer(function()
        ...
        if not okRun then
            warn("[Haryas-Script KeyLoader] main.lua runtime error:", runErr)
        end
    end)
end
