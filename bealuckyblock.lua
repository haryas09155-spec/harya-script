...
local Tabs = {
    ...
    Event = Window:AddTab({ Title = "Event", Icon = "calendar" }), -- FIXED
...
do
            ...
            while eventRunning do
                ...
                for i = 1, 10 do
                    ...
                    if item and item:FindFirstChild("Frame") and item.Frame:FindFirstChild("Free") then
                        ...
                        while eventRunning and locked and locked.Visible do
                            task.wait(0.2)
                        end
                        if eventRunning and claimed and claimed.Visible then
                            continue
                        end
                        if eventRunning and locked and not locked.Visible then
                            pcall(function()
    ...
    local codes = {"release"} -- Add more codes here
    ...
    local buySkin = ReplicatedStorage:WaitForChild("Packages"):WaitForChild("_Index"):WaitForChild("sleitnick_knit@1.7.0"):WaitForChild("knit"):WaitForChild("Services"):WaitForChild("SkinService"):WaitForChild("RF"):WaitForChild("BuySkin")
    local skins = {
        "prestige_mogging_luckyblock", "mogging_luckyblock", "colossus_luckyblock",
        "inferno_luckyblock", "divine_luckyblock", "spirit_luckyblock",
        "cyborg_luckyblock", "void_luckyblock", "glitched_luckyblock",
                            ...
                            for i = 1, 30 do
                                ...
                                if containerFolder and containerFolder:FindFirstChild(tostring(i)) then
                                    ...
                                    if innerModel and #innerModel:GetChildren() > 0 then
                                        local pickupBrainrot = ReplicatedStorage:WaitForChild("Packages"):WaitForChild("_Index"):WaitForChild("sleitnick_knit@1.7.0"):WaitForChild("knit"):WaitForChild("Services"):WaitForChild("ContainerService"):WaitForChild("RF"):WaitForChild("PickupBrainrot")
                    ...
                    pcall(function()
                        if ownedModel and ownedModel.Parent == modelsFolder then
                            ...
                            if ownedModel.PrimaryPart then
                                ...
                                local part = ownedModel:FindFirstChildWhichIsA("BasePart")
                                if part then part.CFrame = newCFrame end
                            end
                        end
                    end)
                    
                    repeat task.wait(0.3) until not autoFarmRunning or (not ownedModel or ownedModel.Parent ~= modelsFolder)
                    if not autoFarmRunning then break end
                    
                    local oldCharacter = player.Character
                    repeat task.wait(0.2) until not autoFarmRunning or (player.Character ~= oldCharacter and player.Character)
                    if not autoFarmRunning then break end
...
do
    ...
    task.spawn(function()
        while true do
            if speedRunning then applySpeed() end
            task.wait(0.2)
        end
    end)
...
-- Event Tab Features
do
    Tabs.Event:AddSection("Easter Event")

    local autoFarmEgg = false
    local eggFolder = workspace:FindFirstChild("Eggs") or workspace:FindFirstChild("EasterEggs") or workspace
    ...
    AFE:OnChanged(function(state)
        ...
        task.spawn(function()
            while autoFarmEgg do
                ...
                if root then
                    -- Search for eggs in common locations
                    for _, v in pairs(eggFolder:GetDescendants()) do
                        if not autoFarmEgg then break end
                        if v:IsA("BasePart") and (v.Name:find("Egg") or v.Name:find("Easter")) and v.CanTouch then
                            -- Teleport to egg
                            root.CFrame = v.CFrame
                            task.wait(0.1) -- Small delay to ensure collection
                        end
                    end
                end
                task.wait(0.5)
